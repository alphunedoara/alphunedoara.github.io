...
function ObjImageActionChangeContents( newImage, newOpacity ) {
...
function ObjImageSizeTo( w, h ){
	...
	if(this.hasOuterShadow)
	{
			...
			xOffset += (((xOffset<0)?-2:2)*this.outerShadowBlurRadius);
			yOffset += (((yOffset<0)?-2:2)*this.outerShadowBlurRadius);
    ...
    this.build()
    this.activate()	
    this.objLyr.clipTo(((yOffset<0)?yOffset:0), w, h, ((xOffset<0)?xOffset:0))
}
...
function ObjImageBuild() {
  ...
  //echo bug 21564
  var bEmbeddedIE8IE9Img = this.x == null ? true : false;
  var radians = 0.0;
  ...
  if(this.hasOuterShadow || this.r>0 || this.vf == 1 || this.hf == 1)
		this.hasEffect =1;//Used for Status indicator naming
  
  xOffset = parseFloat(xOffset.toFixed(5));
  yOffset = parseFloat(yOffset.toFixed(5));

//Build CSS: Depends on browser
/*/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
...
//Add rotation CSS to the image's parent div to browsers other than IE8 and IE9. IE8 and IE9 require a VML rotation attribute on the image level.
/*/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
  if(this.r > 0 || this.vf == 1 || this.hf == 1)
  {
	var tempStr = this.css.substring(0, this.css.length-2);
	tempStr += addRotateCSS(this.r, this.hasOuterShadow, this.w, this.h, this.x, this.y, this.outerShadowDirection, this.outerShadowDepth, this.outerShadowBlurRadius, this.vf, this.hf, this.boundsRectX, this.boundsRectY, this.adornerWidth, this.adornerHeight);
	tempStr += '}\n';
	this.css = tempStr;
  }
  ...
  {	
	...
	{	
		...
		if(xOffset < 0)
		{
			...
			mapOffsetX = xOffset + this.outerShadowBlurRadius;
		...
		if(yOffset < 0)
		{
			...
			mapOffsetY = yOffset + this.outerShadowBlurRadius;
	...
	if(this.bHasClickMap)
	{
		this.divInt += '<g opacity="0.0" transform="translate(' + mapOffsetX + ',' + mapOffsetY + ')">\n';
		this.divInt += '<a id="' + this.name + 'MapArea" style="cursor:pointer;"'
		if( this.hasOnUp && (!is.iOS && !this.bIsWCAG) ) this.divInt += ' xlink:href="javascript:void(null)">\n'
		if( this.hasOnUp && (is.iOS|| this.bIsWCAG) ) this.divInt += ' xlink:href="javascript:' + this.name + '.up()">\n'
		this.divInt += '<path d="' + this.str_SvgMapPath + '"/>\n'
		this.divInt += '</a>\n'
		this.divInt += '</g>\n'
	}
...
function ObjImageDown(e) {
  ...
  if( this.bIsBtn ) 
	this.actionChangeContents(this.imgDn,this.opacityDn);
  this.onSelect()
  this.onDown()
}
...
function ObjImageUp(e) {
  ...
  if( this.bIsBtn ) 
	this.actionChangeContents(this.imgOvr, this.opacityOvr);
...
function ObjImageOut() {
  if( this.bDisabled ) return;
  if( this.bIsBtn )
	this.actionChangeContents(this.imgSrc,this.opacityNorm);
  this.onOut()
}
...
function ObjSetDisabled( bSet )
{
	...
	this.actionChangeContents(img, opacity);	
}
